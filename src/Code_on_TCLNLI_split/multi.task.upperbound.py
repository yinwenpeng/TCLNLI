...
def main():
    ...
    for _ in range(args.repeat_times):
        ...
        for epoch in trange(args.num_train_epochs, desc="train_epochs"):
            ...
            for step, batch in enumerate(tqdm(base_dataloader, desc="BaseTraining")):
                ...
                if step % args.gradient_accumulation_steps == 0 or step == len(base_dataloader) - 1:
                    optimizer.step()
                    lr_scheduler.step()
                    optimizer.zero_grad()

        print('\nBase training over, Start evaluate on unseen_tasks....\n')
        print('first, prepare all dataloader for all unseen tasks')
        evaluate_dataloader_list = []
        for target_task in unseen_tasks:
            ...
            evaluate_dataloader_list.append(target_dataloader)

        '''evaluate'''
        performance_list = []
        for id, target_task in enumerate(unseen_tasks):
            predictions = []
            references = []
            model.eval()
            ...
            for step, batch in enumerate(tqdm(evaluate_dataloader_list[id], desc="Evaluating"+target_task)):
                with torch.no_grad():
            ...
            performance_list.append(rouge_L)

        '''train on all unseen tasks'''
        multi_tasks = [unseen_tasks_pos_path+task_i+'.csv' for task_i in unseen_tasks]
        multi_datasets = load_dataset("csv", data_files={'multitask':multi_tasks})
        column_names = multi_datasets["multitask"].column_names
        text_column = column_names[0]
        summary_column = column_names[1]
        max_target_length = args.max_target_length
        padding = "max_length" if args.pad_to_max_length else False
        with accelerator.main_process_first():
            tokenized_multi_dataset = multi_datasets.map(
                preprocess_function,
                batched=True,
                num_proc=args.preprocessing_num_workers,
                remove_columns=column_names,
                load_from_cache_file=not args.overwrite_cache,
                desc="Running tokenizer on multi dataset",
            )
        tokenized_multi_dataset = tokenized_multi_dataset["multitask"]

        multitask_dataloader = DataLoader(tokenized_multi_dataset, shuffle=True, collate_fn=data_collator, batch_size=args.per_device_train_batch_size)
        multitask_dataloader = accelerator.prepare(multitask_dataloader)
        lr_scheduler = get_scheduler(
            name=args.lr_scheduler_type,
            optimizer=optimizer,
            num_warmup_steps=args.num_warmup_steps,
            num_training_steps=args.num_train_epochs*len(multitask_dataloader),
        )


        logger.info("***** Running  multi task training *****")
        logger.info(f"  Num examples = {len(tokenized_multi_dataset)}")

        model.train()
        for step, batch in enumerate(tqdm(multitask_dataloader, desc="MultiTaskTraining")):
            outputs = model(**batch)
            loss = outputs.loss
            loss = loss / args.gradient_accumulation_steps
            accelerator.backward(loss)
            if step % args.gradient_accumulation_steps == 0 or step == len(multitask_dataloader) - 1:
                optimizer.step()
                lr_scheduler.step()
                optimizer.zero_grad()

        '''evaluate on all unseen tasks again'''
        for id, target_task in enumerate(unseen_tasks):
            predictions = []
            references = []
            model.eval()
    ...
    final_result = computer_mean_std(delta_performance)
    print('Final multitask performance: ', final_result)
